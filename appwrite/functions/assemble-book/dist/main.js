...
async function minimaxChat(messages, maxTokens = 3000) {
    ...
    if (!res.ok)
        throw new Error(`MiniMax erro ${res.status}`);
...
exports.default = async ({ req, res, log, error }) => {
    ...
    try {
        ...
        const citationStyle = bookDoc.citationStyle ?? 'ABNT';
        for (const source of uniqueSources) {
            if (!source.isComplete)
                continue;
            const formatted = citationStyle === 'ABNT'
                ? `${source.authors}. **${source.title}**. ${source.publisher ?? ''}, ${source.publishedAt ?? 'S.d.'}. Disponível em: <${source.url}>. Acesso em: ${new Date(source.accessedAt).toLocaleDateString('pt-BR')}.`
                : `${source.authors} (${source.publishedAt ?? 'S.d.'}). *${source.title}*. Retrieved from ${source.url}`;
            await databases.createDocument(DB_ID, COLL_REFERENCES, node_appwrite_1.ID.unique(), {
                bookId,
                sourceId: source.$id,
                style: citationStyle,
...
4 a 6 parágrafos. Sintetize as contribuições. Aponte limitações e pesquisas futuras. Não inventar referências.`,
        ...
        const chaptersHtml = chaptersRes.documents
            ...
            const content = ch.content
                ? JSON.parse(ch.content)
                : null;
            if (!content)
                return '';
            const sectionsHtml = content.sections
                .map((s) => `<section class="book-section">
  ...
  ${content.conclusion ? `<p class="chapter-conclusion">${content.conclusion}</p>` : ''}
        ...
        const summaryHtml = chaptersRes.documents
            .map((ch) => `<li><a href="#chapter-${ch.chapterNumber}">Capítulo ${ch.chapterNumber}: ${ch.title}</a></li>`)
            .join('\n');
...
<body class="book">
  ...
  <section class="references">
    ...
    <ol>${referencesHtml}</ol>
        ...
        const uploaded = await storage.createFile(BUCKET_ID, node_appwrite_1.ID.unique(), file);
