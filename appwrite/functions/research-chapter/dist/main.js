...
async function searchPerplexity(query) {
    ...
    if (!res.ok)
        throw new Error(`Perplexity erro ${res.status}: ${await res.text()}`);
...
Retorne fontes verificáveis com autor, título, URL e data. NÃO invente referências.`;
        ...
        const rawSources = perplexityData.search_results ?? [];
        ...
        for (const src of rawSources) {
            ...
            const sourceDoc = await databases.createDocument(DB_ID, COLL_SOURCES, node_appwrite_1.ID.unique(), {
                bookId,
                chapterId,
                sectionId: null,
