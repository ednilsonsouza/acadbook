...
async function minimaxChat(messages: Array<{ role: string; content: string }>, maxTokens = 8000) {
  ...
  const data = await res.json() as { choices: Array<{ message: { content: string } }>; base_resp?: { status_code: number; status_msg: string } }
...
Exatamente ${chaptersCount} capítulos, cada um com exatamente ${sectionsPerChapter} seções.`
    ...
    const parsed = JSON.parse(jsonMatch[0]) as { chapters: unknown[] }
