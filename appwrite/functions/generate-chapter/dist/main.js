...
async function minimaxChat(messages, maxTokens = 12000) {
...
exports.default = async ({ req, res, log, error }) => {
    ...
    try {
        ...
        if (!chapter)
            throw new Error(`Capítulo ${chapterNumber} não encontrado no plano`);
        // Buscar fontes do capítulo
        const sourcesRes = await databases.listDocuments(DB_ID, COLL_SOURCES, [
            node_appwrite_1.Query.equal('chapterId', chapterId),
...
Seções:
${chapter.sections.map((s) => `  ${s.sectionNumber}. ${s.title} — ${s.objective}`).join('\n')}
...
Formato JSON:
{
  ...
  "conclusion": "Conclusão do capítulo."
        ...
        const allCompleted = allChapters.documents.every((c) => c.$id === chapterId || c.status === 'completed');
        if (allCompleted) {
